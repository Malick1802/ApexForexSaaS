...
# pandas_ta is a training-only dependency — imported lazily so the dashboard
# can load even if the package is not yet installed in the environment.
try:
...
def triple_barrier_label(
    df: pd.DataFrame,
    stop_loss_pips: float = 40,
    take_profit_pips: float = 80,
    ...
    """
    Apply Dynamic Triple Barrier labeling (1:1.5 R:R).

    Rule 1: SL = max(ATR(14) * 1.5, 25 pips)
    Rule 2: TP = SL * 1.5 (Strict 1:1.5 Risk/Reward)
    Rule 3: Label 1 if TP hit first, 2 if Sell TP hit first, 0 otherwise.

    Args:
        df: OHLCV DataFrame
        stop_loss_pips: Minimum SL in pips (default 40)
        take_profit_pips: Ignored/Deprecated (TP is always 1.5*SL)
    ...
    if not _PANDAS_TA_AVAILABLE:
        raise ImportError(
            "pandas_ta is required for labeling. Install it with: "
    ...
    logger.info(
        f"Labeling with Dynamic 1:1.5 R:R. Min SL={stop_loss_pips} pips, "
        f"TP/SL Ratio=1.5, Max Hold={max_holding_periods}"
    )
    ...
    sl_pips_used = np.zeros(n, dtype=np.float32)
    ...
    for i in range(n - 1):
        ...
        tp_dist = (max(dynamic_sl, min_sl_distance) * 1.5) + spread_dist
        ...
        for j in range(i + 1, end_idx):
            ...
            if long_result == 0:
                ...
                if tp_hit and sl_hit: long_result = -1
            ...
            if short_result == 0:
                ...
                if tp_hit and sl_hit: short_result = -1
    ...
    result['tp_pips'] = sl_pips_used * 1.5
    result['bars_to_outcome'] = bars_to_outcome
