...
st.set_page_config(page_title="Fleet Status Â· ForexAlert", page_icon="ðŸ“Š", layout="wide")
inject_css()
...
def parse_training_log(log_path, tail_bytes=50000):
    ...
    # Keras match: 1/1158 â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â” 50s 40ms/step - accuracy: 0.50 - loss: 0.69
...
def parse_specialist_log(log_path, tail_bytes=50000):
    ...
    # INFO:__main__:âš¡ Optimization Attempt 1/15 for EURUSD 90%+
    # INFO:__main__:âœ… Optimization SUCCESS! 92.1% (Trades: 120)
    ...
    for line in lines:
        ...
        if "Optimization Attempt" in line:
            # âš¡ Optimization Attempt 1/15 for EURUSD 90%+
            try:
        ...
        if "Optimization SUCCESS!" in line or "Candidate" in line:
            # âœ… Optimization SUCCESS! 92.1% (Trades: 120)
...
with st.sidebar:
    ...
    st.markdown("### ðŸ“Š Monitoring Target")
...
if monitor_target == "Global Foundation":
    ...
    no_log_msg = f"â³ No active expert training log found. Expected at: `{log_dir / 'expert_training.log'}`"
...
if not log_files:
    ...
    if status:
        ...
        # Tabs
        tab1, tab2, tab3 = st.tabs(["ðŸš€ Summary", "ðŸ“œ Institutional Audit", "ðŸ› ï¸ System Logs"])
        ...
        with tab1:
            ...
            with col1:
                with st.container(border=True):
                    section_header("ðŸ“Š", "Global Brain Convergence")
                    ...
                    if status["keras_progress"]:
                        ...
                        <div style="display: flex; justify-content: space-between; font-family: var(--font-mono); font-size: 0.8rem; color: var(--text-secondary);">
                            <span>{status['phase']} â€” Step {curr} / {total}</span>
                ...
                with st.container(border=True):
                    section_header("ðŸ›°ï¸", "Live Fleet Preparation")
                    all_symbols = [
            ...
            with col2:
                with st.container(border=True):
                    section_header("ðŸ§ ", "Intelligence Status")
                    ...
                    if st.button("ðŸ”„ Force Refresh", type="primary", use_container_width=True):
        ...
        with tab2:
            with st.container(border=True):
                section_header("ðŸ¦", "Institutional Performance Audit")
        ...
        with tab3:
            with st.container(border=True):
                section_header("ðŸ“œ", "Full System Log Stream")
...
# --- Auto Refresh System ---
st.sidebar.divider()
st.session_state.auto_refresh = st.sidebar.toggle("ðŸ”„ Auto Refresh (30s)", value=st.session_state.auto_refresh)
