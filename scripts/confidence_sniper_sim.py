...
logger = logging.getLogger("SniperSim")

def run_sniper_sim(symbol="GOLD", days=14):
    # 1. Load Specialist
    ...
    if not model_path.exists():
        ...
        logger.info(f"Using Foundation Brain as fallback for {symbol}")
    ...
    thresholds = [0.52, 0.60, 0.70, 0.80, 0.90]
    print("\n" + "="*60)
    print(f" SNIPER SIMULATION: {symbol} (Last {days} Days)")
    print("="*60)
    print(f"{'THRESHOLD':<12} | {'WIN RATE':<12} | {'TRADES':<12}")
    print("-" * 60)
    
    for t in thresholds:
        mask = (confidences >= t) & (y_pred != 1) # Must meet threshold and NOT be "WAIT"
        if np.any(mask):
            wr = np.mean(y_pred[mask] == y_true[mask])
            count = np.sum(mask)
            print(f"{t:<12.0%} | {wr:<12.1%} | {count:<12}")
        else:
            print(f"{t:<12.0%} | {'N/A':<12} | 0")
    print("="*60)

if __name__ == "__main__":
    run_sniper_sim("GOLD", 14)
