...
class InferenceEngine:
    ...
    def predict_symbol(
        ...
        try:
            ...
            # 1. Base Features
            features = self.feature_engineer.extract(df)
            current_price = float(df['close'].iloc[-1])
            
            # 2. Add Context Features (CSM, Bonds, Gold)
            try:
                global_data = self._update_global_context()
                features = self.global_engineer.inject_market_context(features, global_data, symbol)
            except Exception as e:
                logger.warning(f\"Market context injection failed for {symbol}: {e}\")

            # 3. Load Models
            # Specialist Priority (models/specialist/{symbol}/{accuracy}/model.keras)
            models = self.load_models(symbol, target_int)
            
            # Fallback to Phase 3 Expert Adapter if specialist not found
            if not models:
                models = self.load_phase3_expert(symbol)
                
            # Final Fallback to Foundation Model (TFT)
            if not models:
                models = self.load_foundation_model(symbol)

            if not models:
                logger.error(f\"No usable models found for {symbol}\")
                return None

            # 4. Neural Inference
            X_scaled = models['scaler'].transform(features.tail(60 if models.get('model_type') == 'foundation_tft' else 1))
            
            # Reshape for individual model needs
            if models.get('model_type') == 'foundation_tft' or models.get('model_type') == 'expert_adapted':
                 # TFT/Foundation Adaptations expect (1, 60, n_features)
                 X_scaled = X_scaled.reshape(1, 60, -1)
            else:
                 X_scaled = X_scaled.reshape(1, -1)

            buy_threshold = target_int / 100.0
            sell_threshold = target_int / 100.0
            ...
            if signal in ('BUY', 'SELL') and models.get('model_type') not in ('binary', 'expert'):
                ...
                # we force dynamic levels for it even if it's a foundation model bias.
                logger.info(f\"\u26a1 {symbol}: Bypassing Heatmap Gate (Proven/Hurdle Authorized).\")

            # ── 6. Risk Management & TP/SL ──────────────────────────────
            # Entry is last close
            entry_price = current_price
            
            # Dynamic ATR-based levels
            levels = self.calculate_tp_sl(
                symbol, signal, entry_price, 
                atr_pips=features['atr_pips'].iloc[-1] if 'atr_pips' in features.columns else None
            )
            
            # Fetch Trade Volume (How many samples trained this specific accurate model?)
    ...
    def run_continuous(self, interval_minutes: int = 15):
        \"\"\"
        Monitor market and generate signals at regular intervals.
        \"\"\"
        symbols = self.data_engine.get_all_pairs()
        logger.info(f\"Starting continuous inference scan for {len(symbols)} pairs\")
        
        while True:
            try:
                start_time = time.time()
                logger.info(f\"--- Starting market scan at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ---\")
                
                for symbol in symbols:
                    self.predict_symbol(symbol)
                    # Respect API limits
                    time.sleep(1)
                ...
                logger.info(\"Stopping continuous inference engine...\")
                break
            except Exception as e:
                logger.error(f\"Continuous inference crash: {e}\")
                time.sleep(60)
...
if __name__ == \"__main__\":
    ...
    for symbol in [\"EURUSD\", \"GBPUSD\", \"USDJPY\"]:
        res = engine.predict_symbol(symbol, save_to_db=False)
        if res:
            print(f\"{symbol}: {res['signal']} @ {res['confidence']:.2%}\")
